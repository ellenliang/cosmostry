...
";
            ...
            string local_output_filename = System.IO.Path.Combine(temp_folder, "output.tsv");
            ...
            while (true)
            {
                ...
                if (jobinfo.State == VcClient.JobInfo.JobState.Cancelled ||
                    jobinfo.State == VcClient.JobInfo.JobState.Completed ||
            ...
            if (jobinfo.State == VcClient.JobInfo.JobState.CompletedSuccess)
            {
                System.Console.WriteLine("Downloading output TSV tp {0}", local_output_filename);
                VcClient.VC.Download(@"/my/output.tsv", local_output_filename, use_compression, true);
